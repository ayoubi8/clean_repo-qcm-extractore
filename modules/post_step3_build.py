...
def run_post_step3_build(tracker, context, output_user_id: str = None,
    ...
    try:
        ...
        # Surface the stable count + PDF-name workbook generated by Step 5.
        xlsx_src_str = s5_result.get("xlsx_file", "")
        if xlsx_src_str and Path(xlsx_src_str).exists():
            xlsx_dst = Path(step3_accepted) / Path(xlsx_src_str).name
            shutil.copy2(xlsx_src_str, xlsx_dst)
            print(f"[AUTO-BUILD] ✅ Copied xlsx → step3_metadata/accepted/{xlsx_dst.name}")
